...
醫院情境分成「院內設施／物流」與「臨床接觸」兩層。前者可沿用幾何、感知、避障與事件規劃；後者還需要材料、接觸力、軟組織、醫療影像與醫療器材風險管理。專案因此以風險遞增順序建立七個**教學代理**：

| 階段 | 案例 | 已計算內容 | 尚未進入模型 |
|---:|---|---|---|
| H1 | 檢體試管轉移 | 軌跡、傾角、槽位誤差、相鄰距離 | 條碼模型、液體晃動、破裂 |
| H2 | 無菌器械盤 | 語意區域、器械放置、違規幀 | 微生物、消毒與包裝完整性 |
| H3 | 床旁管路整理 | 管路中心線、張力代理、彎曲半徑、禁區 | 校正管材、呼吸移動、病患連接 |
| H4 | 病區配送 | 人員距離、等待、門／電梯事件 | 行為預測、多車隊、真實設施 API |
| H5 | 消毒覆蓋 | 網格相對劑量與覆蓋率 | 入射角、陰影、風場、殺菌效力 |
| H6 | 假體探頭掃描 | 接觸力與形變代理 | FEM、聲學、超音波影像、組織分類 |
| H7 | 導管路徑預覽 | 中心線、插入深度、力代理 | Cosserat rod、血流、血管形變、透視影像 |

Open-RMF 的官方 demos 已提供兩層 clinic、電梯及多車隊案例，可作 H4 接入 ROS 2／Gazebo 的後續基礎；SOFA 專注生物力學與軟組織交互作用，並有探頭—組織接觸參考案例，適合 H6 由解析代理升級時評估。H7 可參考 SurRoL 的 dVRK／PyBullet／Taichi 研究環境，但本專案目前沒有重現其手術或 sim-to-real 成果。當功能意圖開始涉及病患診斷、治療或醫療器材時，必須另行依 ISO 14971 建立危害、風險控制與驗證流程；本頁門檻不是該流程的替代品。

這個順序的核心理由是：物理引擎只能讓錯誤的座標、機構尺寸或路徑以更昂貴的方式失敗。先用幾何時間序列驗證 `TrajectoryFrame`，再讓 MuJoCo 產生同一份逐幀介面，較容易區分「資料／規劃錯誤」與「接觸物理錯誤」。

### 已落地的軟管學習情境

`configs/motions/hose_extraction_demo.json` 定義一根 49 節點軟管、三根固定管路、11 個原始動作關鍵幀與 5 mm 安全餘量。規劃器會插入 1 個「自動安全繞行」關鍵幀，預設執行產生 116 幀、9.6 秒的連續動作：
...
- 具有關節限制的阻尼最小平方六自由度 IK，以及 flange/TCP 工具偏移。
